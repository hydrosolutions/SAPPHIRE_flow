...
SAPPHIRE Flow is an operational hydrological forecasting system that ingests weather and station data, runs ensemble forecast models, checks alert thresholds, and serves results via a REST API with an optional review dashboard. Currently in **mid-implementation** — Phases 1a/1b (types, Protocols, test infra, DB schema), 2 (stores), 4 (services), 7 (model framework + training), 10 (Docker Compose) are complete. Phase 5 (station onboarding) is partial (steps 1–5 done; steps 6–8 blocked on 7b). Phase 3 (adapters) is ~30% (protocols + fakes + converters; no production or replay adapters). **Next up: Phase 7b** (model onboarding + sample model — the key blocker), then Phase 3 remainder, Phase 6 (observation ingest), Phase 9 (FastAPI API), Phase 8 (forecast cycle). See `docs/v0-scope.md` §H for the full phase ladder. The immediate goal is **v0**: a working end-to-end pipeline using Swiss public data (MeteoSwiss weather, BAFU stations) with simple models, validating the architecture before Nepal deployment in Oct 2026.
