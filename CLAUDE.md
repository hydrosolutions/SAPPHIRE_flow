...
- **Planning and independent-review agents may read a DRAFT plan, but implementation agents may not execute it.** ⚖️ **READY is set by the ORCHESTRATOR session** (delegated by the owner, 2026-09-17); see § Orchestration authority.
- **Multi-model review is mandatory for all non-trivial plans and patches** (trivial-only exemption: typos, comments, single-line log text, mechanical no-behavior edits). The owner deliberately starts one independent Claude and one independent Codex pass; no model approves its own output. High-risk work receives one additional owner-commissioned review. The human owner decides findings, PR approval and merge; the orchestrator sets READY (§ Orchestration authority). See `docs/workflow.md` § Multi-Model Review.
...
complete review; there is no confirmation mode. The ORCHESTRATOR sets `status: READY`
(§ Orchestration authority); no other agent may.

## Orchestration authority (delegated 2026-09-17)

The owner delegated day-to-day sequencing to a single **orchestrator session**, which is
accountable for reaching v1: daily Nepal forecasts running on the staging host, on the six real
DHM stations. What that does and does NOT include:
...
escalate — do not set it yourself.
⛔ **Nothing reaches the project without the owner**: the orchestrator never merges.
Escalation runs agent → sub-orchestrator → orchestrator → owner; escalate rather than guess.
