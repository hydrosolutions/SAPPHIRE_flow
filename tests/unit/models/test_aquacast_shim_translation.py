"""Plan 181 T1/T2/T3 — the shim's FI-surface proxying, declaration rewrite, and
boundary data translation, exercised WITHOUT the `aquacast` extra.

Split out of `test_aquacast_shim.py` by the Plan 181 fixer: every test in this file
was previously behind `pytest.importorskip("aquacast", ...)` even though none of it
actually needs the real package — `_shim.py`'s translation functions are pure
functions of `forecast_interface` types, and `AquacastShim.__init__` is the ONLY
thing that touches `aquacast` (it imports it lazily, inside the constructor). CI's
required `unit` job runs `uv sync --frozen` without the `aquacast` extra
(`.github/workflows/ci.yml`), so the whole suite was silently skipped there while CI
stayed green.

Instead of constructing a real `CmalPoolPT()` and swapping its `_inner`, these tests
build an `AquacastShim` instance via `object.__new__` (bypassing `__init__`, so
`importlib.import_module("aquacast...")` never runs) and inject a FAKE inner and a
SYNTHETIC native requirement shaped like the real `cmal_pool_PT` declaration. The
translation functions under test (`_canonical_requirement`, `_to_aquacast_inputs`,
`_to_canonical_result`, ...) don't care whether that shape came from a parsed YAML or
a fixture — they are pure transformations of `InputRequirement`/`ModelInputs`/
`ModelResult`.

What did NOT move here — construction against the REAL vendored config, and real
`discover_models` registration — stays in `test_aquacast_shim.py`, gated behind the
`aquacast` extra, because those genuinely need the real package.
"""
...
def _native_requirement() -> fi.InputRequirement:
    """A SYNTHETIC mirror of the real `cmal_pool_PT` declared requirement's shape
    (verified against the real package while writing this split): `discharge`
    target + past in `mm/day`, `precipitation` past/future in `mm/day`,
    `mean_temperature` past/future in `degC`, ONE daily branch, `area` among the
    statics. Enough structure to exercise T2/T3 translation without the extra.
    """
        ...
        dynamic={
            _DAILY: fi.SpatialInputSpec(
                data={
                        ...
                        past_known={
                            "aquacast": {
                                "discharge": _past(fi.Unit.MM_PER_DAY),
                                "precipitation": _past(fi.Unit.MM_PER_DAY),
                                "mean_temperature": _past(fi.Unit.DEG_C),
                            }
                        },
                        future_known={
                            "aquacast": {
                                "precipitation": _future(fi.Unit.MM_PER_DAY),
                                "mean_temperature": _future(fi.Unit.DEG_C),
                            }
                        },
                    )
                }
...
def _requirement_with_precip_at(time_step: timedelta) -> fi.InputRequirement:
    """A fabricated requirement carrying a `precipitation` future_known variable at
    `time_step`. Used ONLY to exercise the D1 daily-step guard: the real
...
def _quantile_discharge_success(
    values_by_level: dict[str, dict[str, float]],
) -> fi.ModelResult:
    """Like `_discharge_success`, but with a `quantiles` container (>=3 levels, the
    FI minimum) instead of `deterministic`. `cmal_pool_PT` declares BOTH heads and
    SAP3 prefers quantiles over deterministic — a deterministic-only fixture cannot
    detect broken/deleted quantile scaling (Plan 181 fixer finding)."""
    levels = [0.1, 0.5, 0.9]
    variables: dict[str, dict[str, fi.VariableOutput]] = {}
    for station_key, by_level in values_by_level.items():
        frame = pl.DataFrame(
            {
                "issue_datetime": [_ISSUE],
                "datetime": [_VALID],
                **{str(level): [by_level[str(level)]] for level in levels},
            }
        ).with_columns(
            pl.col("issue_datetime").cast(pl.Datetime("us", "UTC")),
            pl.col("datetime").cast(pl.Datetime("us", "UTC")),
        ...
        variables[station_key] = {
            "discharge": fi.VariableOutput(
                ...
                quantiles=fi.QuantileData(quantile_levels=levels, data=frame),
                status=fi.VariableStatus.SUCCESS,
            )
        }
    return fi.ModelSuccess(
...
class _StubAquacastShim(AquacastShim):
    """A construction-free double: built via `object.__new__` so
    `AquacastShim.__init__`'s `importlib.import_module("aquacast...")` never runs.
    Only `_model_name`/`_inner` are set, matching exactly what `__init__` would have
    populated — no `CONFIG_FILENAME`/`model_tier`/`alert_eligibility` needed since
    none of these tests go through `__init__` or `discover_models`."""


def _shim_with_fake_inner(**fake_kwargs: Any) -> AquacastShim:
    shim = object.__new__(_StubAquacastShim)
    shim._model_name = "cmal_pool_pt"  # noqa: SLF001 -- matches CONFIG_FILENAME's stem
    fake_kwargs.setdefault("input_requirement", _native_requirement())
...
class TestFISurfaceDelegation:
    ...
    def test_train_reaches_the_inner_model_and_delivers_translated_inputs(
        ...
        """Plan 181 fixer: proving delegation is not enough — T3 requires the
        DELIVERED inputs to be name/unit/value-translated, so a pass-through
        (unconverted) delegation must not satisfy this test."""
        sentinel = object()
        ...
        received = shim._inner.train_calls[0]["inputs"]  # noqa: SLF001
        ...
        # 1.0 m3/s over 864 km2 is exactly 0.1 mm/day — a pass-through (1.0) or a
        # wrong-direction conversion (10.0) would both fail this.
    ...
    def test_hindcast_raises_clearly_when_the_inner_model_has_none(self) -> None:
        shim = object.__new__(_StubAquacastShim)
        shim._model_name = "cmal_pool_pt"  # noqa: SLF001
        shim._inner = _FakeInnerWithoutHindcast(  # noqa: SLF001
            input_requirement=_native_requirement()
        )
        inputs = fi.ModelInputs(
            stations={
                "gauge-a": _canonical_station_inputs(
...
class TestDeclarationTranslation:
    ...
    must not otherwise alter the contract's shape. `_canonical_requirement` is a pure
    function of whatever `InputRequirement` it is given, so exercising it against the
    synthetic `_native_requirement()` fixture proves the SAME translation logic the
    real `cmal_pool_PT` config drives.
    """
    ...
    def test_every_declared_unit_has_a_sap3_canonical_mapping(self) -> None:
        ...
        req = _shim_with_fake_inner().input_requirement
...
class TestPrecipitationRelabelGuard:
    ...
    8x. Covers BOTH the declaration path (`_translate_declared_unit`) and the actual
    data-translation path (`_aquacast_value_and_unit`) — Plan 181 fixer finding: only
    the former was guarded, leaving the latter free to silently mislabel.
    """

    def test_refuses_a_non_daily_precipitation_relabel_in_the_declaration(
        self,
    ) -> None:
        shim = _shim_with_fake_inner(
    ...
    def test_refuses_a_non_daily_precipitation_relabel_in_the_data_path(self) -> None:
        shim = _shim_with_fake_inner()
        non_daily = timedelta(hours=3)
        dynamic_inputs = fi.DynamicInputs(
            past_known={
                "aquacast": {"precipitation": _series("precipitation", 5.0, fi.Unit.MM)}
            },
            future_known={},
        )
        station_inputs = fi.StationInputs(
            dynamic={
                non_daily: fi.SpatialInputs(
                    data={fi.SpatialRepresentation.BASIN_AVERAGE: dynamic_inputs}
                )
            },
            static={"area": 86.4},
        )
        inputs = fi.ModelInputs(stations={"gauge-a": station_inputs})
...
class TestBoundaryTranslationData:
    ...
    def test_outbound_translation_scales_every_quantile_column(self) -> None:
        """Plan 181 fixer: `cmal_pool_PT` declares BOTH deterministic and quantile
        heads, and SAP3 prefers quantiles over deterministic — this exercises
        `_scaled_data_container` on a REAL `QuantileData` (>=3 levels) so a broken
        or deleted quantile-scaling branch fails a test."""
        shim = _shim_with_fake_inner(
            predict_result=_quantile_discharge_success(
                {"gauge-a": {"0.1": 1.0, "0.5": 2.0, "0.9": 3.0}}
            )
        )
        inputs = fi.ModelInputs(
            stations={
                "gauge-a": _canonical_station_inputs(
                    area_km2=864.0, discharge_m3_s=0.0, precip_mm=0.0, temp_c=0.0
                )
            }
        )
        ...
        variable = result.output.variables["gauge-a"]["discharge"]
        assert variable.metadata.unit is fi.Unit.M3_PER_S
        quantiles = variable.quantiles
        assert quantiles is not None
        assert quantiles.quantile_levels == [0.1, 0.5, 0.9]
        # 1.0/2.0/3.0 mm/day over 864 km2 is exactly 10.0/20.0/30.0 m3/s.
        assert quantiles.data["0.1"][0] == pytest.approx(10.0)
        assert quantiles.data["0.5"][0] == pytest.approx(20.0)
        assert quantiles.data["0.9"][0] == pytest.approx(30.0)
        # Temporal columns must be untouched by the value scaling.
        assert quantiles.data["issue_datetime"][0] == _ISSUE
        assert quantiles.data["datetime"][0] == _VALID
...
class TestAreaMissingAtPredictTime:
    ...
    def test_missing_area_returns_a_model_failure_naming_the_station(self) -> None:
        ...
        assert shim._inner.predict_calls == []  # noqa: SLF001 -- inner must NOT run

    def test_non_finite_area_returns_a_model_failure(self) -> None:
        shim = _shim_with_fake_inner()
        station_inputs = _canonical_station_inputs(
            area_km2=float("nan"), discharge_m3_s=1.0, precip_mm=0.0, temp_c=0.0
        )
        inputs = fi.ModelInputs(stations={"thun": station_inputs})
        ...
        assert isinstance(result, fi.ModelFailure)
        assert result.cause is fi.FailureCause.INPUT_DATA
        assert shim._inner.predict_calls == []  # noqa: SLF001

    def test_hindcast_missing_area_returns_a_model_failure_naming_the_station(
        self,
    ) -> None:
        shim = _shim_with_fake_inner()
        station_inputs = _canonical_station_inputs(
