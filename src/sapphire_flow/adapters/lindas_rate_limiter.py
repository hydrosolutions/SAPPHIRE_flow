...
``call()`` starts its clock before token acquisition, not after, and re-checks
the remaining budget before every attempt (including the first) AND again
after token acquisition (which can itself wait) — a starved bucket counts
against the deadline exactly like a slow retry sequence does.

The deadline means what retry deadlines normally mean: **no NEW attempt starts
past it**, and no sleep runs past it. It does not abort an HTTP request already
in flight — each individual attempt is bounded by the HTTP client's own
configured connect/read/write/pool timeouts. An earlier version ran ``send`` on
a daemon thread joined with a hard timeout to get a stricter bound; that
bounded the *caller's* wait but abandoned the thread and its in-flight request,
so repeated timeouts leaked threads and live LINDAS requests while reporting
the call as exhausted. Bounding the wait while the work continues unseen is a
worse guarantee than an honest one, so the thread is gone. ``send`` receives
``remaining_s`` and may tighten its own client timeouts if a caller ever needs
the harder bound.
...
class TokenBucketLindasLimiter:
    ...
    def call(self, send: Callable[[float], httpx.Response]) -> httpx.Response:
        ...
        while True:
            ...
            try:
                # `send` runs on the calling thread. An earlier version ran it
                # on a daemon thread joined with a hard timeout; that bounded
                # the CALLER's wait but abandoned the thread and its in-flight
                # request, so repeated timeouts leaked both. The deadline now
                # means what retry deadlines normally mean: no NEW attempt
                # starts past it. Each individual attempt is bounded by the
                # HTTP client's own configured phase timeouts, and
                # `remaining_s` is passed so a caller may tighten those
    ...
    def _acquire_token(self, *, deadline_remaining_s: float) -> None:
        ...
        earlier version held the lock across `self._sleeper(...)`, which
        serialized every waiter behind every other waiter's full sleep: a
        caller's own deadline was not evaluated until it finally acquired the
        lock, so the total-deadline guarantee could be blown before anything
        noticed. Re-checking under the lock after each wait keeps the
    ...
    def _drain(self) -> None:
        """Zero the local bucket — used when a 429 proves the upstream bucket
        is actually empty. Refills forward from now, so the next independent
        ``call()`` only gets credit for time genuinely elapsed after the
        drain.

        The clock is read INSIDE the lock. Reading it outside meant a caller
        that then blocked on the lock could hand `_refill` a timestamp older
        than `_last_refill`, moving it backward and granting phantom refill
        credit — defeating the very drain this performs.
        """
        with self._lock:
            now = self._clock()
            self._refill(now)
    ...
    def _refill(self, now: UtcDatetime) -> None:
        """Credit elapsed time to the bucket. Caller must hold `_lock`.

        Monotonic by construction: a non-advancing `now` is ignored entirely
        rather than rewinding `_last_refill`, so time can never be credited
        twice.
        """
