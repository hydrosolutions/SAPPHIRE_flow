...
| D4 | **Single CVE scanner, two layers**: `trivy fs .` in the lint tier (scans `uv.lock` + `pyproject.toml` without building the image — fast feedback on dep-only PRs) and `trivy image <tag>` after the build (catches OS-level CVEs). Both fail CI on HIGH+ CRITICAL findings **with an available fix**; `--ignore-unfixed` excludes the rest. *(Corrected 2026-08-17, Plan 180 T3: this row previously said "Both fail CI on HIGH+ unfixed", which contradicted the shipped `--ignore-unfixed` config and left two authoritative docs disagreeing — see `docs/standards/security.md` § CVE scanning layers for the reconciled policy.)* | Earlier draft had `pip-audit` + `trivy`. Trivy already reads `uv.lock` in `fs` mode, so `pip-audit` duplicates ~80% of coverage with negligible Python-advisory edge. One tool means one config, one ignore-list (`.trivyignore`), and one set of CI steps to reason about. Drop the duplication. |
