...
- **Cross-reference (Plan 065):** once the config-overlay mechanism lands (Plan 065 DONE, landing this session as commit TBD), the rebase of `staging-5-stations` onto main described below is superseded by running A3 directly from main with `docker compose -f docker-compose.yml -f docker-compose.staging.yml up` (selects the `config/overlays/staging-5-stations.toml` overlay). Existing procedure retained here unchanged for in-flight sessions. See `docs/plans/archive/065-*.md` after this plan archives.
