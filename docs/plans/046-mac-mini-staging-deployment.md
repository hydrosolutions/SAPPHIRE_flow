...
- `ensemble_size`: `len(fc_result.ensemble.members)` for the primary `ForecastEnsemble` on `fc_result` — the same ensemble that was just stored. If the station fan-out produced multiple model results, log once per `(station, model)` and take `ensemble_size` from each `ForecastEnsemble` individually.
- `lead_time_hours`: `len(fc_result.ensemble.timesteps) * fc_result.time_step.total_seconds() / 3600` — computed off the same ensemble. For v0a daily models this resolves to `5 * 24 = 120`; for v0b+ hourly 120-step models it resolves to `120`.
...
Restore the full `[onboarding]` list (167 original + 2033 + 2085 = 169). Repeat A3 end-to-end (including `onboard-model` as step 1).
...
| `config.toml` | A1 (transient, branch only) | 5-station subset (four rivers + one lake) on `staging-5-stations` local branch; never pushed, never opens a PR. |
...
4. **`staging-5-stations` branch management**: confirm local-only, never pushed, never merged — only the permanent 2033/2085 addition lands on main.
