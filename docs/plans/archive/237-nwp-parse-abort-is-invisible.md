...
general gap-marking overhaul (that is Plans 253/250 territory), or a fourth task.
...
253/250 — T1 fails before a record is built).
...
- `converters.py:119` hardcodes `is_gap=False` for every record. T1 sidesteps it by failing earlier;
  the underlying gap-marking gap belongs to Plans 253/250.
