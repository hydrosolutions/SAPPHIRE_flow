...
| **D3** | What if `area` is missing at predict time? | **RESOLVED, corrected by the fixer round (2026-08-18): return `ModelFailure`, naming the station — NOT raise.** The original resolution ("raise") violated the mandatory FI rule that an anticipated `predict`/`hindcast` input failure must be RETURNED, never raised (`CLAUDE.md` § ForecastInterface Adherence). `_units.py::_require_area` still raises internally (now a dedicated `AreaConversionError`, a `ConfigurationError` subtype), but `AquacastShim.predict`/`hindcast` catch exactly that subtype at the translation boundary and return `ModelFailure(cause=FailureCause.INPUT_DATA, message=...)` naming the station — before `self._inner` is ever reached. `train` is unaffected: the FI protocol's `train` returns `TrainedArtifact`, not `ModelResult`, so it has no `ModelFailure` path and correctly keeps raising. **No fallback to a basin lookup** — a fallback would silently reintroduce D2's mismatch at exactly the moment the intended value is absent. |
