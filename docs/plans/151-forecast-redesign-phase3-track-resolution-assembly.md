...
edit approved, D21 global sweep confirmed — **no open forks remain**).
...
1. **Exact-51 is a source-derived property, never a literal.** 51 is ECMWF-IFS-only (fc member 0 + 50 pf,
   ...
   (`docs/architecture-context.md:1469`; `adapters/meteoswiss_nwp.py:968-972`, `_MIN_ENSEMBLE_MEMBERS` at `:38`).
   `min_operational_ensemble_size` (`config/deployment.py:123`) is an alert/onboarding **output** floor
   (`services/alert_checker.py:184`, `services/model_onboarding.py:437-453`) and is **never** a statement about a
...
2. **Exactly ONE forcing track per assignment.** Multi-track assignments (multi-product / mixed-mode requirements) are
   a named follow-on, rejected at registration by T2's conformance sweep. Grounding: `grep -rn "swe\|snow"
   src/sapphire_flow/models/` returns **zero hits** — no model consumes snow as a `future_known` feature; snow is
...
3. **Per-feature horizons are RETAINED** (`FeatureFetchHorizons`), but **acceptance is evaluated at the deduped
   ...
   *scalar* was considered and rejected: per-variable horizons are named inside build-sequence item 3 of the ratified
   redesign, and the FI cross-branch `max` collapse (`adapters/forecast_interface.py:474-481`) flattens a multi-branch
   model even when each branch is internally uniform. The earlier per-consumer-acceptance variant of this ruling was
   **withdrawn on 2026-08-11** because it is unrepresentable under the locked one-cycle-per-track contract
   (`docs/spec/types-and-protocols.md:3063-3068,3088-3090`); see D26 for the reversal and its cost.
...
1. **Per-feature horizon (and mode) is lost — three collapses.**
   - *Cross-assignment superset collapse.* `build_superset_requirements` (`services/operational_inputs.py:324`) unions
     every assigned model's features and takes the **max** step count (`:377-378`), and rejects a mixed
     SINGLE/ENSEMBLE station outright (`:351-358`). The flow builds this superset once per station
     (`flows/run_forecast_cycle.py:2036`). "precip 2 steps + temp 10 steps" becomes a rectangular 10-step demand.
   - *Within-FI-requirement collapse.* The FI adapter irreversibly `max`-collapses per-variable `future_steps` into one
     scalar `forecast_horizon_steps`, **across every `time_step` branch** (`adapters/forecast_interface.py:474-481`).
   ...
   - *Consumption-side (runner) collapse.* Even with per-feature fetch and per-assignment assembly, `_run_single_model`
     ...
     `forecast_horizon_steps` (`services/run_station_forecast.py:172-179`), fan-out from the global mode (`:243`), and
     the whole `future_dynamic_features` set to `fan_out_ensemble` (`:287`). Phase 3 therefore carries the per-feature
     contract into the runner (D10).

2. **One assembly / one shared cycle per station.** `assemble_station_operational_inputs`
   (`services/operational_inputs.py:384`) reads NWP for one `cycle_time` (`:492`) and produces one frame for every
   assignment. The flow threads one `nwp_cycle_reference_time` (`flows/run_forecast_cycle.py:1935-1955`) into every
...
3. **No exact-member-set enforcement.** `assess_future_coverage` (`services/nwp_coverage.py:116-117`) checks only that
   ...
   partial ensemble (`recap_gateway.py:842-871`).
...
MeteoSwiss-fed** (`flows/run_forecast_cycle.py:1578`) and stays on the legacy best-effort superset path, so for it
...
- **Group operational ENSEMBLE fan-out**; group **CONTROL** stays on the legacy superset path fed by the Phase A
  `_fetch_nwp_task` (`flows/run_forecast_cycle.py:1842-1854`), read back at `nwp_readback_cycle_time` (`:2413`).
- **Member survival** — v1 is all-members-or-fail.
- **Hindcast/operational parity** — `services/hindcast.py` uses a separate assembler (`:134`, called `:331`) with
  `prior_state=None` (`:358`).
...
- **D2 — Pure track projection returning a discriminated result.** A `ModelAssignment` (`types/station.py:66-72`)
  ...
  - **The SELECTED time-step branch is authoritative.** FI permits a `DynamicInputSpec` containing only `past_known`,
    ...
    features across all branches (`forecast_interface.py:458`). `NoForcingRequired` is therefore decided from **the
  ...
  - **`NoForcingRequired`** covers fallback and skill models with no future forcing (`models/persistence_fallback.py:48`,
    `models/linear_regression_daily.py:52`). These have no track; their context is assembled independently of track
    resolution, so a fallback still runs when forcing tracks fail.
  - **Non-FI models** broadcast their single scalar `forecast_horizon_steps` (`types/model.py:269`) across every
    declared future feature (`:265`).
...
- **D4 — Enforced FI subset, rejected at registration.** Phase 3 supports and **enforces at the FI boundary**: single
  spatial representation (already enforced, `forecast_interface.py:488-492`); **exactly one non-empty `future_known`
  product per selected time-step/spatial branch**; and **one `ensemble_mode` per branch**. A violating requirement
  raises `ConfigurationError` at construction, so Flow 12 onboarding surfaces it before a cycle runs — a documented,
  enforced narrowing, never a silent one (`CLAUDE.md` § FI Adherence). Scope cost is recorded in D21.
  - **`past_known` products are NOT counted** (reviewer blocker-fix). `SeasonalPrecipRunoffRegression` declares a
    **second** `past_known` product alongside the single `future_known` one (`models/nwp_regression.py:765`), while
    ...
    FI models declare exactly one `future_known` product (`:211`), so none is rejected by the rule as now written.
- **D5 — Track deduplication → exactly ONE resolved cycle per track.** `resolve_tracks()` groups `ForcingRequired` by
  ...
  (`docs/spec/types-and-protocols.md:3088-3090` — one `resolved_cycle: UtcDatetime` field, no per-assignment cycle).
...
- **D6 — `CandidateAwareForecastSource` + `isinstance` dispatch.** `@runtime_checkable` is mandatory: the dispatch is
  ...
  non-conforming. `expected_member_ids(track)` is the **single derivation point** for the source's raw member identity.
...
- **D7 — Per-track resolution = walk-back policy in `services/`, driven at flow level.** A pure service walks back from
  ...
  today it reaches only the production factory branch (`flows/run_forecast_cycle.py:453-462`) while the injected-client
  branch (`:463-466`) silently uses the constructor default.
  ...
  - **Per-HRU containment is required of the migrated adapter** (verified against `main`). Today the control (`fc`)
    fetch sits outside any containment block (`recap_gateway.py:825`) while only the `pf` loop tolerates
    `RecapDataUnavailableError` (`:842-871`), so an unavailable control fetch for HRU A aborts before HRU B can remain
    available — contradicting this plan's central rule that one station must not reject a candidate for another.
    `fetch_requirement` must contain temporal unavailability per HRU, retain successful siblings, and reserve a
    candidate-wide `ABSENT_INCOMPLETE` for a payload in which **no** in-scope station is complete (D8). Containment is
    about the *fetch*: `fetch_requirement` returns whatever each HRU yielded, per station, without judging it — the
    completeness verdict is the services-side gate's job (D8), never the adapter's.
  - **Missing polygon column = per-station unavailability on the per-track path; the legacy path is unchanged**
    ...
    `AdapterError` when any resolved polygon lacks a response column (`recap_gateway.py:511-525`). That fail-loud
    ...
    **explicit** per-station unavailability with reason `MISSING_POLYGON_COLUMN` (T1), while complete siblings survive.
    ...
    (`docs/spec/types-and-protocols.md:3082`), and Phase 3 does not widen it. The original concern is answered, not
  ...
  - **Configuration errors stay candidate-wide, never per-station** (D27). Duplicate-polygon resolution raises
    `RecapConfigurationError` with **zero** Gateway calls (`tests/unit/adapters/test_recap_gateway.py:640`,
  ...
  - **Phase A must not persist partial candidates for migrated stations.** The legacy `_fetch_nwp_task` still runs for
    the group path, and the legacy `fetch_forecasts` returns partial ensembles by design (`recap_gateway.py:842-871`).
...
- **D8 — Completeness is a PER-STATION predicate; the candidate verdict is derived from it.** Two gates that must not
  ...
  1. **Raw completeness — evaluated per `(station, feature, member, horizon)`.** For the pre-extracted recap path the
     ...
     (`recap_gateway.py:821-878` — `acc` is station-keyed and the `pf` loop `break` at `:869-871` deliberately keeps a
     *partial* member set), so "the candidate" has no single global member axis: a candidate-wide union of member ids
     ...
     per-feature group MAX** (`docs/spec/types-and-protocols.md:3063-3068`) — so "5-day + 10-day dedup onto one track"
     ...
     - **"Steps" are counted in the TRACK's `time_step` units, after the same reduction assembly uses** (reviewer
       ...
       **daily** buckets, and assembly applies issue-time filtering plus per-feature aggregation and bucketing before
       counting (`services/operational_inputs.py:130,211,521,535`). Counting raw rows would accept a candidate with
       ...
       (`operational_inputs.py:211-234`), not `>`.
  ...
  - **Accepted cost (recorded, not hidden):** an incomplete station does **not** get its own walk-back — the track's
    ...
    single `TrackFetchResult.resolved_cycle` (`docs/spec/types-and-protocols.md:3088-3090`). See D29.
  - **`StationUnavailableReason` gains `INCOMPLETE_AT_CYCLE`** (and `MISSING_POLYGON_COLUMN`, D27) — the spec's enum
    (`docs/spec/types-and-protocols.md:3082-3085`) carries only `NO_DATA_AT_CYCLE | EXTRACTION_EMPTY | NOT_SUBSCRIBED`
...
- **D9 — Per-assignment assembly.** Each assignment assembles its own frame from its track's available records at the
  ...
  `_variables_over_nan_tolerance` evaluates both past and future inputs (`forecast_interface.py:622`) and
  `_past_known_nan_tolerances` still iterates every branch (`:592`), so both tolerance maps must derive from the one
  selected `DynamicInputSpec`. Per-assignment `nwp_age_hours` is recomputed from **that assignment's** resolved cycle
...
- **D10 — Runner consumption seam via a discriminated input.** "Receive a context, but also detect an absent one" is
  ...
  fields from **that** provenance rather than the shared runner arguments (`run_station_forecast.py:144-145,:392-393`).
...
- **D11 — Cross-cycle combination = fail-loud preflight; same-cycle stays green.** The check runs at the **top of the
  per-station persist block**, before the individual-forecast store (`flows/run_forecast_cycle.py:2214-2227`) and the
  state store (`:2228-2243`) — not at the combination point (`:2246`), where a mismatch would leave partial writes
  ...
  - **Trackless combinable models are neutral.** `LinearRegressionDaily` is a non-fallback skill model with no future
    ...
    (`run_station_forecast.py:110`). Equality is compared only across **non-null** forcing cycles; the preflight fails
...
- **D13 — Observation fetch stays parallel; logging.** Observation fetch runs concurrently with track projection and
  ...
  at `flows/run_forecast_cycle.py:1857` runs parallel to Phase A at `:1842`). New canonical `{entity}.{action}` events:
...
lands its production change **and its test consumers together** — no task leaves a known failure. Per-task exit gate is
the focused modules plus full `uv run pytest -q`, `uv run pyright`, and `uv run ruff check`.
...
- **T1 — Domain types + additive failure causes (D1).**
  - **In scope:** create `types/forcing_track.py` with the D1 types (no `OutputHorizon`); add `MISSING_CONTEXT` and
    ...
    **Also in scope (docs-with-code):** the corresponding corrections to `docs/spec/types-and-protocols.md` —
    ...
    members, the per-`(station, feature, member, horizon)` wording of the completeness gate (`:3063-3068`,
    `:3082-3085`), the `OutputHorizon` deferral, **and the `fetch_requirement` return-type change from
    `CandidateFetchResult` to the raw fetch outcome (`:2984-2987`, `:3092`), with `CandidateFetchResult` constructed
    services-side (D31)** — so READY ratifies these authoritative contract changes rather than deferring them.
    **`TrackFetchResult` keeps its single `resolved_cycle` field unchanged** (`:3088-3090`): Phase 3 does **not**
...
- **T2 — FI accessors + conformance sweep (D3, D4).**
  - **In scope:** per-time-step public accessors on `ForecastInterfaceAdapter` returning the selected branch's
    ...
    raising `ConfigurationError` for multi-product or mixed-mode branches. Note there are **three** FI entry points
    (`pyproject.toml:141`), not one — the sweep is repo-wide (D21).
  - **Red-first (accessor tests belong in the existing `tests/unit/adapters/test_forecast_interface_adapter.py`):** a
    ...
    inheritance); a **past-only branch** exposes empty future horizons and does **not** raise from a mode accessor;
    multi-product and mixed-mode branches raise `ConfigurationError`; the existing multi-spatial guard is unchanged.
...
- **T4 — Source contract + adapter migration (D6, D7 containment).**
  ...
  - **Red-first:** a conformance test proves `fetch_requirement` requests **exactly** the nominal cycle and performs no
    ...
    **a two-HRU test where HRU A's control fetch is unavailable and HRU B still resolves**; **a missing polygon column
    for one station yields that station's explicit unavailability while its sibling's rows survive** (today this raises
    batch-wide, `recap_gateway.py:511-525`); **a duplicate-polygon misconfiguration still raises
    ...
    `tests/unit/adapters/test_recap_gateway.py:640`); a transport failure is **raised** rather than returned as a
...
- **T5 — Per-track resolution: walk-back policy + completeness + `TrackFetchResult` (D7, D8).**
  - **In scope:** the pure fetch/validate half (walk-back bounded by `max_cycle_age_hours` threaded on **both**
    ...
    conversion boundary between the candidate payload and the records the validator reads (existing conversion lives at
    `flows/run_forecast_cycle.py:1236`).
...
- **T6 — Per-assignment assembly (D9).**
  ...
  - **Red-first:** an assignment needing precip 2 steps and temp 10 assembles a frame where precip's short horizon is
    ...
    until `_run_single_model` (`run_station_forecast.py:231`), which T7 owns.
...
- **T8 — Flow wiring + golden tests + fail-loud combination (D11, D12, D13).**
  - **In scope:** wire `flows/run_forecast_cycle.py` to project and dedup tracks, resolve each track sequentially,
    ...
    and `_fetch_nwp_task` (`flows/run_forecast_cycle.py:936-942`) sets no `retries=`, so T4's "raise transients so a
  ...
  - **Overlap rule: a station that is BOTH group-member and per-track-eligible stays on the LEGACY path in Phase 3**
    ...
    (`flows/run_forecast_cycle.py:2317,2343`), so the two re-scoping rules can collide on one station. The earlier
    ...
    (`flows/run_forecast_cycle.py:2407`, `services/run_group_forecast.py:128`). Suppressing Phase A's write would drop
  ...
  - **Exit gate (whole repo):** full `uv run pytest -q` + `ruff` + `pyright`, plus
    `uv run pytest tests/integration/test_e2e_pipeline.py -m slow -q` — explicitly overriding the default `not slow`
    exclusion (`pyproject.toml:111`) so the end-to-end control-only path actually runs.
...
- **D10-completeness-pre-extract** — for the pre-extracted recap path the gate boundary is **before persist**; the
  ...
  which a station-keyed pre-extracted candidate does not have (`recap_gateway.py:821-878`), so the gate is stated
...
- **D28-max-retries-wiring** (new, 2026-08-11 review; **superseded by the Codex gate — now IN SCOPE at T8**)
  ...
  `@task(retries=...)` reads it and `_fetch_nwp_task` (`flows/run_forecast_cycle.py:936-942`) sets no `retries=`. It was
...
- **D29-per-station-cycle-deferred** (new, 2026-08-11 review) — a station that is incomplete at the track's accepted
  ...
  unrepresentable under the locked single `TrackFetchResult.resolved_cycle` (`docs/spec/types-and-protocols.md:3088-3090`).
...
- **D24-cadence-source** (owner, 2026-08-11; **corrected after review**) — the walk-back candidate cadence comes from
  **configuration**, not a third Protocol member. The original wording added "recap already carries the constant it
  ...
  `max_cycle_age_hours` is threaded.
- **D26-horizon-axis** (owner, 2026-08-11; **revised 2026-08-11 after review**) — per-feature horizons are
  **retained**. Collapsing to a scalar was considered and rejected: per-variable horizons are named inside
  build-sequence item 3 of the ratified redesign, and the FI cross-branch `max` collapse flattens a multi-branch model
  even when each branch is internally uniform.
  **Reversal (the review was right):** the earlier form of this ruling also rejected "accept at the group max" and
  ...
  `ResolvedTrackRequest.fetch_horizons`, the MAX (`docs/spec/types-and-protocols.md:3063-3068`), and gives
  `TrackFetchResult` a **single** `resolved_cycle` (`:3088-3090`) — two assignments sharing a track cannot settle on two
...
- **D21-conformance-sweep-scope — RESOLVED (owner-ratified 2026-08-11): option (A), keep the GLOBAL sweep.** T2's
  ...
  requirement. **Ratified consequence:** such a model fails at **registration** — loudly, at onboarding — even when it
  is assigned only to stations that would never take the per-track path. Preferred over option (B) (gating the sweep
  behind the migrated-adapter dispatch), which defers the identical failure to mid-cycle on a live station. Nothing
  in-repo trips it today: all three shipped FI models declare exactly one `future_known` product
  (`models/nwp_regression.py:211`). **Check aquacast (Plan 152) against this rule before it onboards** — it is the
  most likely candidate to declare a shape the sweep rejects.
...
- **D25-group-snow-scoping** — `_compute_required_snow` (`flows/run_forecast_cycle.py:781-810`) excludes group
  requirements, as its own docstring records (`:795-797`). Phase 3 **does not fix this**: it is pre-existing on `main`,
...
- **D27-polygon-parser-fail-loud — RESOLVED here** (superseding the earlier "resolve during
  ...
  - **Missing polygon column** (a data gap) → **per-station** `StationTrackUnavailable`, new reason
    ...
    carries an enum-valued `reason` with no free-text payload (`docs/spec/types-and-protocols.md:3082`), and Phase 3
...
- **D28-fetch-retries — IN SCOPE at T8** (superseding the earlier "cut/deferred" note). T4's design raises typed
  ...
  today; `flows/run_forecast_cycle.py:936-942` sets no `retries=`). Without it a single transient terminates the track.
- **D31-candidate-ownership — SPEC EDIT, folded into T1** (reviewer blocker-fix). The spec pins
  `fetch_requirement(...) -> CandidateFetchResult` (`docs/spec/types-and-protocols.md:2984`) whose `status` is the
  **completeness** taxonomy (`:3092`), but the adapter cannot judge completeness — the threshold is a `services/`-side
...
**Assumption (flagged, not a fork):** Plan 150's `AssignmentFailureCause` extends cleanly — new members plus new
production sites only. If the confirming pass finds the enum is consumed by an exhaustive `match` without a wildcard,
T7 must add the new arms.
