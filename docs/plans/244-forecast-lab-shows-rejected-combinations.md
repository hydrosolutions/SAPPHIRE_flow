...
scope: Add quality status and flags to the combined-forecast objects in the Forecast Lab snapshot format, which requires a v2 to v3 transition across every producer and checker of the version string. Explicitly NOT the storage of a failed combination (Plan 242 T2a), NOT any change to QC rules, NOT the member-model objects.
depends_on: [242]
blocks: []
source: 2026-09-04 — owner decision OD-1a in Plan 242, taken once review established the cost of surfacing
---
...
**DRAFT — not reviewed.** Split out of Plan 242 on 2026-09-04 by owner decision, so that a versioned
external-format change does not ride on a persistence fix.

## Why this exists

Plan 242 makes a combined (`_pooled`) forecast that fails QC **stored, marked failed** rather than
dropped (OD-1), on the reasoning that the combination exists to compare models and a dropped row
leaves no record of what was rejected. Plan 242 then has to **exclude that row from the Forecast
...
- Storing the failed combination — that is Plan 242 T2a and must land first.
...
**Outcome:** a `QC_FAILED` combination appears in the Lab, visibly marked failed, and Plan 242's
exclusion filter is removed rather than left dormant.

**In:** `services/forecast_lab/db_sources.py:184-204` (the exclusion Plan 242 T2a adds) and the
...
**Pre-change:** with Plan 242 landed, a snapshot built over a cycle whose combination failed QC contains no combined entry for it at all.
...
2. **Plan 242 has landed first.** Surfacing a row that is not yet stored would test nothing.
