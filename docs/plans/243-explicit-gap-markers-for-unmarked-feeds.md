...
depends_on: [242]
blocks: []
source: 2026-09-03 — owner decision recorded in Plan 242 § Decision
---
...
**STUB — DRAFT, not scoped, not reviewed.** Created so that Plan 242's decision has somewhere to
...
WMO-168 Vol I. Plan 242 relabels that claim; this plan is what would make it true.
...
this is the low-risk option, is in `docs/plans/242-close-the-store-boundary-seam.md` § Decision — it is
not repeated here.
...
- **⛔ Downstream consumers beyond QC — the expensive-to-discover-late one.** Materialising null rows
  ...
  Plan 242's newly persisted `input_quality`; hindcast forcing; skill sample size; and Forecast Lab.
