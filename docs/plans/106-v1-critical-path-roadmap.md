...
| **Historical training-forcing backfill window** (parametric, multi-year) | *to-draft* (fold into **082 Task 3B**) | gap | NOW | `ingest_weather_history` is **hardcoded to a 60-day window** (`ingest_weather_history.py:51,300` — the MeteoSwiss open-data archive limit); Nepal ERA5-Land/Snowmapper **training** needs a **parametric multi-year** window + gateway back-extraction trigger. Mechanism (parametric window) is Swiss-testable NOW; the multi-year *data* is gateway-backfill (manual runbook, §3). Without it, Flow 6 cannot accumulate training forcing. | 1 |
...
no phase-2 manifest change (`082:437`); the
