...
- **D1 — A SECOND read contract, frozen as strictly as the first.** `ImergReadContract`
  (`imacquire.py:277`) and `contract_from_open_granule` (`:336`) stay untouched and keep serving the
...
- **D3 — Retrieve the FULL window: 2020-01-01 → 2025-12-31.** At 2.7 GB there is no reason to sub-select,
  ...
  ⚠️ **Retrieval is ~105,216 requests** — that is the real cost here, not bytes. Rate-limit politely,
  make it resumable, and record gaps as gaps (Plan 220's rule: a gap is data, a wrong retrieval is not).
...
- **D5 — The archive route stays, and stays the reference.** ⛔ Do not delete it. One granule already on
  ...
  that granule**: the subset's 4,500 cells must equal the corresponding cells of the full field, value
  for value. ⛔ If they do not, the subset route is wrong and this plan stops.
...
**Verify:** all 4,500 cells equal within float tolerance. ⛔ **A mismatch stops the plan** — report it
rather than adjusting a tolerance to pass.
