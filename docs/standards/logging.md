...
| Event | Level | Notes |
|---|---|---|
| `forecast.input_quality_assessed` | INFO if `input_quality == "partial"`, WARNING if `input_quality == "degraded"` | **Specified, not implemented** (Plan 242 T4b, 2026-09-04): neither forecast path (`services/run_station_forecast.py`, `services/run_group_forecast.py`) emits this event -- `assess_input_quality()` runs and the result is now persisted and served (Plan 242 T1a-T1c), but nothing logs it. Level-conditional as designed: emitted when input quality is not FULL, not emitted when quality is FULL. Kwargs: `input_quality` (str), `flags` (list of dicts: `{"category": str, "level": str, "detail": str}`). Bind `model_id` and `station_id` via `bind_contextvars` before this event so both appear automatically. Emitting it is outside Plan 242's persist-and-serve scope. |
