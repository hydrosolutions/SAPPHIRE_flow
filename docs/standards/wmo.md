...
| **Automated range + temporal-consistency checks** | WMO-168 Vol I | `_apply_range_check` and `_apply_rate_of_change` (`services/qc.py:110`, `:131`), implemented by `Stage1QualityChecker` (`services/qc.py:288`). Runnable: `uv run pytest tests/unit/services/test_qc.py`. **Scope of this evidence:** checker behaviour. That the checker runs against every ingested observation is a separate claim, evidenced by the ingest flow's own tests, not by this row. | 2026-09-24 |
| **Input-quality metadata persisted and exposed through the API** | WMO-1072, QMF-H | `InputQualityLevel`/`InputQualityFlag` persisted on `forecasts` (migration `0054`) and read back by value, not by dataclass default. Runnable: `uv run pytest tests/integration/db/test_migration_input_quality.py tests/integration/store/test_forecast_store.py tests/unit/api/test_api_stations.py tests/unit/api/test_api_forecasts.py`. **Scope:** persistence and authenticated API serialisation ONLY. | 2026-09-07 |
...
**Specified, not verified:** *"Forecasters informed when a forecast is produced under degraded input conditions"* (WMO-1072, QMF-H). The metadata reaches the API — see the verified row above — but an endpoint existing does not establish that a forecaster was informed. The dashboard indicator and the `forecast.input_quality_assessed` log event are both specified and unimplemented (`docs/standards/logging.md`, `docs/architecture-context.md:110`). This row moves to verified when an operational forecaster-facing consumer is evidenced, not before.
