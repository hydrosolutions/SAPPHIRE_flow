...
- `sapphire_dg_api_key` — recap Data Gateway API key (Nepal v1). Wired into `docker-compose.yml` (`services.prefect-worker.secrets`, `services.prefect-worker-ingest.secrets`, top-level `secrets.sapphire_dg_api_key.file: ./secrets/sapphire_dg_api_key`) by Plan 082 Task 2A. Read at runtime via `config.recap_gateway.load_recap_api_key()`, which falls back to the `RECAP_API_KEY` env var for local dev (same pattern as `db_password`/`DB_PASSWORD` in `docker/entrypoint.sh`). Absent on Swiss deployments (the Recap adapters are never constructed there — Task 2C/2D `type` selector) — **operators on Swiss hosts must still create an (empty or placeholder) `./secrets/sapphire_dg_api_key` file**, because both worker services declare the secret unconditionally and Docker Compose refuses to start a service whose declared `secrets:` file is missing.
