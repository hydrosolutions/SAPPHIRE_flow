...
**`recap-dg-client` distribution policy** (decided 2026-06-25, follows the ForecastInterface precedent above): the `hydrosolutions/recap-dg-client` Data Gateway client will be distributed by **git pin plus a scoped exception to the wheel-only guard** (the same Plan 079-style bridge used for `forecastinterface`), not a bespoke mechanism. Unlike `forecastinterface`, `recap-dg-client` is a **private** repository, so both GitHub-hosted CI and the Docker builder stage need **clone auth** (a deploy key or scoped token) to resolve the git pin. The follow-up removal trigger mirrors ForecastInterface's: migrate off the git pin to a versioned **private-index wheel** (a Plan 080-style hydrosolutions package index) and drop the source-build exception once that wheel is published. The actual `pyproject.toml` git pin and the CI wheel-only-guard exception land in **Plan 082** (where live Gateway smoke needs the committed dependency); **Plan 081 adds no committed dependency** and stays offline-testable against an injected fake client.
