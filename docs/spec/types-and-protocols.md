...
class OperationalForecast:
    ...
    input_quality: InputQualityLevel | None = None  # None = unknown (no assessment recorded -- legacy row, Plan 253 T1a/T1b); never defaults to FULL
...
class ForecastStore(Protocol):
    ...
    def fetch_forecast_summaries(
        ...
        *,
        model_id: ModelId | None = None,
        parameter: str | None = None,
        degraded_only: bool = False,  # Plan 253 T1c/OD-2 -- PARTIAL/DEGRADED only, unknown (NULL) never matches
        limit: int = 50,
        offset: int = 0,
