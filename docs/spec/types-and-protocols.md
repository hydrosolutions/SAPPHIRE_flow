...
**Flow 1 integration note** — Step 1.10: Forecast output QC. Runs `ForecastQualityChecker.check()` on each ensemble. Aggregate `QC_FAILED` raises `SanityCheckFailure` (flow tries fallback model). `QC_PASSED` or `QC_SUSPECT` results are stored on the `OperationalForecast`. For hindcasts, `QC_FAILED` flags the hindcast but does not trigger fallback. **Exception (Plan 246 T2a, OD-1):** a combined (`_pooled`/`_bma`) forecast that fails QC is STORED marked `QC_FAILED` rather than routed to fallback -- it has no next candidate to fall through to, so dropping it would forfeit the evidence of what was rejected and why.
...
class OperationalForecast:
    ...
    input_quality: InputQualityLevel | None = None  # None = unknown (no assessment recorded -- legacy row, Plan 246 T1a/T1b); never defaults to FULL
...
class ForecastStore(Protocol):
    ...
    def fetch_forecast_summaries(
        ...
        *,
        model_id: ModelId | None = None,
        parameter: str | None = None,
        degraded_only: bool = False,  # Plan 246 T1c/OD-2 -- PARTIAL/DEGRADED only, unknown (NULL) never matches
        limit: int = 50,
        offset: int = 0,
