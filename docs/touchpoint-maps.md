...
- additive attribute-only writer: `PgBasinStore.merge_namespaced_attributes` (Plan 155 T1b) — a THIRD basin-write path, deliberately separate from `store_basin`/`update_basin_from_package`: a JSONB `||` merge into `basins.attributes` only, no new `basin_versions` row, no `material_change`, no affected-artifact set. Structurally guarded to reject any key not carrying its `prefix` (default `caravan:`, D15), so it cannot touch an existing non-namespaced attribute even by mistake — this is what makes skipping the supersede/flag machinery sound. Orchestrated end-to-end by `store/caravan_import.py::import_caravan_attributes` (joins Caravan's `caravan_camels_ch_<code>` parquet rows onto `(network, code)` stations). Touch this when changing additive-vs-correction write semantics for basin attributes.
...
- Caravan-declared static resolution (Plan 155 T2, G8): `services/caravan_statics.py` (`resolve_caravan_static_key` / `available_declared_static_keys` / `project_declared_static_attributes`) — a Caravan-trained model (e.g. `cmal_pool_PT`) declares statics under Caravan's own names, which live `caravan:`-namespaced in `Basin.attributes` (D15, no bare-name fallback). Wired into EVERY compatibility-and-frame boundary that used to read `basin.attributes` directly: `flows/onboard_model.py::_validate_compatibility_task`, `services/model_onboarding.py::onboard_model`'s own compatibility step, `services/training_data.py::assemble_station_training_data`'s missing-static gate + static-frame build, `services/hindcast.py::_load_static_attributes`, `services/operational_inputs.py::assemble_station_operational_inputs`. Needs no change to `adapters/forecast_interface.py::_static_inputs` — the projection always resolves into the model's own declared BARE column name.
