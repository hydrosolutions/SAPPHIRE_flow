...
- freshness heartbeat (Plan 116): `_emit_forecast_freshness_record` — writes
  ...
  deployment** map). Fixer round: the group forecast-store loop's
  `store_forecast` call no longer special-cases `StoreError` with a
  re-raise — it is contained exactly like the station path, so a total
  forecast-store outage in a group-only deployment still reaches this
  emitter instead of crashing the flow before it.
