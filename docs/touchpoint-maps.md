...
- Store/state failure handling is **not uniform** — it differs by call
  ...
  REFUSAL (rows 1-3, `ForecastRetryConflictError`) follows each path's
...
- **`store_forecast` classifies a re-run before inserting** (Plan 327). The table carries a partial unique index (`uq_forecasts_station_model_issued_param`, whose `status <> 'superseded'` predicate is **unreachable today** — `ForecastStatus` has no such member, so it behaves as a FULL unique index; Plan 328 owns making it reachable). A write whose natural key `(station_id, model_id, issued_at, parameter)` already exists is classified by `services/forecast_retry.py::classify_forecast_retry` (the ordered decision table, first match wins): **row 4 (identical)** returns the STORED `ForecastId`, writes nothing and raises nothing; **rows 1-3** (values differ / artifact differs / QC verdict differs) raise `ForecastRetryConflictError`. ⛔ Evidence state is NOT consulted — every forecast carries an incomplete-evidence marker, so classifying on it would refuse every resume.
