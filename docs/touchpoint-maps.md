...
- freshness heartbeat (Plan 116): `_emit_forecast_freshness_record` — writes
  ...
  **Plan 237 T2**: the `nwp_outcome is None or fetch_failure_reason is not
  None` abort branch (`:2668`) now RAISES `ForecastCycleAbortedError`
  (`exceptions.py`) immediately after writing this CRITICAL record,
  instead of returning `ForecastCycleResult(health=FAILED)` normally —
  Prefect was reporting a zero-forecast NWP abort as `COMPLETED`, silently
  disagreeing with the watchdog's own CRITICAL alert for the same event.
  The raise happens at the FLOW level, never inside `_fetch_nwp_task`
  (which has already swallowed its exception into a returned outcome by
  this point); no `retries` are configured on `run_forecast_cycle_flow`,
  so this does not trigger a re-fetch. Five tests that previously asserted
  on the returned `ForecastCycleResult` for this path now wrap the call in
...
- Store/state failure handling is **not uniform** — it differs by call
  ...
  changing it. **Plan 327 adds a THIRD outcome to both paths:** an IDENTICAL
  re-run of an already-stored forecast (decision-table row 4) is not a failure
  at all — `store_forecast` returns the stored identity, writes nothing and
  raises nothing, which is what makes a died-partway cycle re-runnable. A
  **Plan 328 then turns rows 1 and 2 into SUPERSESSIONS** — the stored row is
  marked `superseded` and the replacement written in the same transaction, so
  neither path sees a failure and the cycle proceeds on the new content. A
  REFUSAL (⛔ **row 3 only**, `ForecastRetryConflictError`) follows each path's
  existing asymmetry: the STATION path degrades AND withholds that
  `(station, model, parameter)` from alert selection; the GROUP path re-raises
  and aborts the cycle before Phase C.
...
- **Transaction scope differs by caller and is not symmetric.** API writes run inside `engine.begin()` (one commit/rollback per request); flows run on an AUTOCOMMIT connection, so **each statement commits on its own** and multi-statement writes are **not atomic as a unit** — `store_hindcast`, `store_group` (group + members), and `store_artifact` (filesystem then DB row) can partial-write on a crash. Diff `_db.py` before assuming a change relies on atomicity. ⛔ **`store_forecast` is the EXCEPTION and was listed here in error** (corrected by Plan 327 T2): `PgForecastStore` defaults `self._begin` to `conn.engine.begin` (`store/forecast_store.py`), so header, values, evidence and blobs share ONE transaction whatever the caller's connection mode — proven by `tests/integration/store/test_forecast_store.py::test_values_insert_failure_rolls_back_header`, and migration `0028` cleaned the historical orphans. A forecast header without its values **cannot occur**; do not build repair paths for one.
...
- **`store_forecast` classifies a re-run before inserting** (Plan 327). The table carries a partial unique index (`uq_forecasts_station_model_issued_param`, `WHERE status <> 'superseded'`), **reachable since Plan 328 T1 / migration `0058`** added `ForecastStatus.SUPERSEDED` and widened the `forecasts.status` CHECK (`ck_forecasts_status`). ⛔ The predicate itself was NOT changed — it became reachable, which is what lets a replacement occupy the natural key its predecessor held. A write whose natural key `(station_id, model_id, issued_at, parameter)` already exists is classified by `services/forecast_retry.py::classify_forecast_retry` (the ordered decision table, first match wins): **row 4 (identical)** returns the STORED `ForecastId`, writes nothing and raises nothing; **rows 1 and 2** (`SUPERSEDING_ROWS`) mark the stored row `superseded` and insert the replacement IN THE SAME TRANSACTION, returning the replacement's id; **row 3** (`REFUSING_ROWS`) raises `ForecastRetryConflictError`. ⛔ Reference the rows BY NUMBER — Plan 327 § D1 is the only place the conditions are written down, and every prose restatement of them so far has drifted. ⛔ Evidence state is NOT consulted — every forecast carries an incomplete-evidence marker, so classifying on it would refuse every resume. ⛔ The superseded row's evidence and blobs are NEVER touched: migration `0057`'s triggers reject `UPDATE`/`DELETE`/`TRUNCATE` on both evidence tables.
- **CURRENT reads exclude a superseded forecast; BY-ID reads keep it** (Plan 328 T3). `fetch_latest_forecast`, `fetch_forecasts_for_cycle`, `fetch_latest_uncombined_issued_at` and an unfiltered `fetch_forecasts_in_range` all filter on `status <> 'superseded'` — the first two had **no status filter at all** before this plan, and the Forecast Lab calls both and takes the first candidate (`services/forecast_lab/db_sources.py`). ⛔ `fetch_forecast`, `fetch_evidence` and `fetch_forecast_summaries` are DELIBERATELY unfiltered: the evidence is permanent, so the forecast carrying it must stay readable, and the record listing is where its id is discovered. A new reader of `forecasts` must state which side it is on.
- **`ForecastRetryConflictError` is the ONLY translated store-write failure.** Plan 038 D5 stands: everything else still propagates as a raw SQLAlchemy exception, including the `IntegrityError` from a CONCURRENT duplicate that slips between the classification SELECT and the INSERT. Do not widen the translation; do not assume a retry-on-`SapphireError` caller covers a general storage outage.
- **A refused (row 3) forecast must not be alerted on — via TWO mechanisms, because one cannot reach a combination.** `store_forecast`'s return value is discarded by `flows/run_forecast_cycle.py` and Phase C alerts on the in-memory ensemble. A refused INDIVIDUAL model is dropped from `all_ensembles` (`_drop_refused_ensembles`). 🔴 A refused COMBINATION (`_pooled`/`_bma`) is **never a key in that dict** — it holds the contributors, and `PooledEnsembleStrategy` rebuilds the pool from them — so deleting it there is a silent no-op; instead the `(station, parameter)` pair is passed to `check_station_alerts(refused_combinations=...)`, which forces PRIMARY for it and logs `alert.strategy_degraded` with `reason="combination_refused"`. A new store-refusal path must cover both or the cycle acts on content the store would not keep.
- **A resumed contributor must be rebound before a combination's evidence is built.** `capture_combined_evidence` copies each contributor's in-memory `forecast_id` AND `evidence_sha256`, and `_combined_contributor_gap` checks both against PERSISTED evidence. The flow's `_persist_forecast` re-fetches the persisted evidence (fetching the forecast does not hydrate it) and rebinds both. Honest historical absence — a contributor predating migration `0057` — still yields `contributor_evidence_not_persisted`, correctly, and must not be suppressed.
...
- **No Pg SQL store wraps SQLAlchemy exceptions** — raw `sqlalchemy.exc.*` propagates out of the store layer. (Plan 327's `ForecastRetryConflictError` is not a counter-example: it is raised from a semantic pre-check BEFORE the write, never by catching a SQLAlchemy error.) `StoreError` is raised by `ZarrNwpGridStore` and by the **caller / service layer** (e.g. group-forecast, hindcast), **not** by the Pg stores, and there is no transient-vs-fatal classification inside them. Any such classification lives in the caller (see the store-failure asymmetry in the **Forecast cycle** map), not here.
...
- **Compose topology**: `docker-compose.yml` — services (`postgres`, `prefect-server`, `prefect-worker`, `prefect-worker-ingest`, `api`, `caddy`, one-shot `init`), named volumes, `backend`/`frontend` nets, `cap_drop:[ALL]`, and `image: sapphire-flow:${VERSION:?…}` on every built service **except `prefect-worker`, which since Plan 262 T2 builds its OWN `sapphire-flow-aquacast:${VERSION}` tag** (the only image carrying the `aquacast` extra / torch) — so a deploy builds TWO images, and a change to the `x-app-build` anchor reaches the other four ONLY. The built services are `prefect-worker`, `prefect-worker-ingest`, `prefect-worker-backup`, `api` and one-shot `init`. **Three file-based DB password secrets** (Plan 147 Slice D): `db_password` (owner — mounted only into `postgres`/`prefect-server`/`init`), `sapphire_api_db_password` (mounted into `api` + `init`), `sapphire_worker_db_password` (mounted into both workers + `init`) — `api`/the workers mount ONLY their own role's secret. **All built services are `read_only: true`** — a new write path needs an explicit `tmpfs:` / volume or the container fails at start.
...
- **`partition_by_time_step_and_phase` now returns `(cohorts, rejected_count)`, not just `cohorts` (Plan 235 per-run scope, blocker #1).** The rejected count (hindcasts excluded by the previous bullet's internally-mixed-phase check) never reached any caller's completeness accounting before this — a run that silently dropped a malformed INPUT during partitioning still reported every partitioned cohort complete and published a SUBSET as if it were the whole generation. `compute_skills_task`, `compute_combined_skills_task` (summed across every combined model), and `services.onboarding._compute_skill` all fold this into their own completeness gate alongside `cohorts_missing`/`cohorts_combined` — do not call this function and discard the second element. **Generation identity is now content-derived, not the raw flow-minted id (Plan 235 per-run scope, blocker #3, reopens `docs/decisions/plan-235-skill-generations.md`'s D1):** `services.skill.service.resolve_generation_id(invocation_id, scores, diagrams)` hashes the actually-computed score/diagram VALUES and combines that with the flow-minted invocation id (`uuid5`) to derive the row identity actually stored — an unchanged-input Prefect retry re-derives the SAME id (safe collision), a changed-input retry (e.g. a corrected observation) derives a DIFFERENT one, so it can never lose to `ON CONFLICT DO NOTHING` against an earlier, orphaned attempt's stale rows. **Generation-tagged writes are gated by `DeploymentConfig.enable_skill_generations` (default `false` — corrected by the independent-review fixer round; a `true` default would have Release A silently emitting generation-tagged writes before every rollback image understands them) for a two-release rollout (Plan 235 per-run scope, blocker #4)** — `false` (Release A) keeps `compute_skills_task`/`compute_combined_skills_task`/`services.onboarding._compute_skill` on the pre-235 baseline shape (`generation_id=NULL`, no `skill_generations` publish) so a rollback mid-rollout never serves mixed generations to a generation-unaware reader; an operator opts INTO generation-tagged writes only for Release B, once every rollback image in the fleet is generation-aware. Every test that exercises the generation-aware write path must now pass `deployment_config=make_deployment_config(enable_skill_generations=True)` explicitly — a bare `deployment_config=None`/omitted no longer implies it.
- **`compute_generation_fingerprint` (`services/skill/service.py`) now covers every persisted semantic field, not just a score's scalar `score` (independent-review fixer round, blocker).** It previously omitted `flow_regime_config_id` (on both scores and diagrams) and a diagram's own `data` JSONB blob entirely — a retry whose only difference was a corrected diagram or a reconfigured flow-regime boundary re-derived the SAME `resolve_generation_id` output and silently lost to `ON CONFLICT DO NOTHING` against the earlier attempt's stale row. `d.data` is now folded in via `json.dumps(..., sort_keys=True, default=str)` so key order never perturbs the digest. Do not drop either field when extending the fingerprint — anything a reader can observe on a published row belongs in it.
- **A BMA cohort requires BOTH cross-validation folds to produce output (independent-review fixer round, blocker).** `compute_bma_skill_cross_validated` (`services/skill/combined_skill.py`) checks `fold1_complete`/`fold2_complete` (each `bool(scores or diagrams)`) BEFORE calling `_average_skill_scores`/`_merge_fold_diagrams` — previously, one fold legitimately returning `([], [])` (e.g. no usable BMA weights from that fold's training half) let the OTHER fold's single, un-cross-validated result publish as if two-fold CV had succeeded. On an incomplete fold the function returns `([], [])`, which the existing flow-level completeness gate (`compute_combined_skills_task`'s `cohorts_combined` count) already treats as "this cohort did not combine" — no separate flow-level wiring needed.
- **An orphan `hindcast_forecasts` header (no matching `hindcast_values` rows) now RAISES `StoreError` for a run-scoped fetch, not just a log-and-skip (independent-review fixer round, blocker).** `PgHindcastStore.fetch_hindcasts`/`fetch_hindcasts_by_station` still log-and-skip an orphan header for an UNSCOPED fetch (`hindcast_run_id`/`hindcast_run_ids` omitted) — but every production caller through `compute_skills_task`/`compute_combined_skills_task` (T3: both required) is ALWAYS scoped, and a silent drop there happens upstream of every rejected-hindcast completeness gate (blocker #1 above), so a real storage-layer gap would otherwise publish an undercounted "complete" generation. Do not add a broad `except StoreError` around either fetch in the flow layer — the failure is meant to surface loudly.
- **`uq_skill_scores_natural_key`/`uq_skill_diagrams_natural_key` now include `model_id` and a NULL-safe `model_artifact_id` (fixer round, BLOCKER fix, migration 0052 amended in place).** *(Same partial-index caveat as the previous bullet: this tightened shape applies only to `computation_version >= 2`.)* Migration 0052 (previous two bullets) widened both natural keys to include `(time_step_seconds, phase_offset_seconds)` but still indexed `model_artifact_id` RAW (no `COALESCE`) and never indexed `model_id` at all. `services/skill/combined_skill.py::compute_combined_skill` always computes pooled/BMA scores with `model_artifact_id=None` — PostgreSQL treats every `NULL` as distinct from every other `NULL`, so (a) two repeated pooled/BMA computations for the identical stratum never collided under `ON CONFLICT DO NOTHING` and inserted a duplicate row on every re-run, defeating idempotency entirely, and (b) without `model_id` in the key, `POOLED_MODEL_ID` and `BMA_MODEL_ID` scores would have collided with each other the moment `model_artifact_id` was made NULL-safe. `model_id` is now part of both natural keys and `model_artifact_id` is compared via `COALESCE(model_artifact_id::text, '')`. Proved by two real-Postgres integration tests (`tests/integration/store/test_skill_store.py`): `test_repeated_pooled_computation_with_null_artifact_is_idempotent` and `test_pooled_and_bma_null_artifact_scores_do_not_collide` — both proven RED against the pre-fix schema (the first via unbounded duplicate inserts, the second specifically against a COALESCE-but-no-`model_id` half-fix).
- **The tightened natural key is a PARTIAL index (`computation_version >= 2`), with a second, separate legacy index preserving 0051's guarantee below it (Plan 228 per-run scope, 2026-09-03, BLOCKER fix, migration 0052 amended in place, no new migration).** An intermediate version of this migration added `eval_period_end` to both keys as a manufactured per-run recompute identity — **that approach is RETRACTED** (independent design check, 2026-09-03): a safe recompute identity also requires every consumer to read the newest generation and an atomic mark-and-replace covering diagrams (there was none), which is a bigger, interconnected design now owned by **Plan 235** (must land before Plan 228's D3 recompute executes). `PgSkillStore.fetch_latest_scores`/`fetch_latest_diagrams` and their `FakeSkillStore` mirror are back to the plain `max(computation_version)` selection; a later, same-version recomputation of the same stratum again collides under `ON CONFLICT DO NOTHING` — a known, accepted gap until Plan 235 closes it. What this round actually fixes: a plain (non-partial) `CREATE UNIQUE INDEX` over the tightened, `COALESCE`-NULL-safe key would **fail on deploy**, because the live mac-mini's ~115,000 `computation_version = 1` scores legitimately contain repeated NULL-`model_artifact_id` duplicates under 0051's looser (raw, non-`COALESCE`) guarantee. Both `uq_skill_scores_natural_key`/`uq_skill_diagrams_natural_key` are now partial (`WHERE computation_version >= 2`), grandfathering that legacy generation untouched — deletes nothing. Closing the hole this opens (a future `computation_version < 2` write, reachable via `docs/standards/cicd.md`'s one-version rollback compatibility, would otherwise have ZERO uniqueness protection): each table keeps a SECOND partial index, `uq_skill_scores_natural_key_legacy`/`uq_skill_diagrams_natural_key_legacy` (`WHERE computation_version < 2`), in EXACTLY 0051's original shape — a future v1 write is still deduplicated exactly as before, no better, no worse. Proved by `tests/integration/db/test_migration_0052_partial_index.py::test_upgrade_survives_legacy_null_artifact_duplicates` (RED: `UniqueViolation` against a plain tightened index; GREEN once partial) and `tests/integration/store/test_skill_store.py::test_legacy_version_writes_still_deduplicated_after_partial_index_cut` (RED with the legacy index removed: both v1 duplicates survive uncollided; GREEN with it present).
- **`onboarding.py::_make_skill_fn`'s `_compute_skill` now partitions by `(time_step, phase)` before scoring, matching the two `flows/compute_skills.py` tasks (major fix).** It fetches a station/model's ENTIRE `training_period_start..end` window unfiltered by `hindcast_run_id` — the same unsafe shape the flow tasks were fixed to avoid — and `store_hindcast` has no natural-key dedup (previous bullet: "a re-run is additive, not idempotent"), so a training window containing an earlier onboarding's hindcasts at a different `time_step`/phase used to hit `validate_homogeneous_time_step_and_phase`'s hard-raise and fail the ENTIRE onboarding unit (`FAILED_SKILL`) instead of degrading to "fewer cohorts scored at onboarding". Proved by `tests/unit/services/test_onboarding.py::TestMakeSkillFnMixedCadence`, proven RED against the pre-fix unpartitioned call.
...
- **Skill scores/diagrams now carry a `generation_id` and a `skill_generations` publication ledger (Plan 235, migration 0054) — the recompute-identity gap the previous bullet's `Plan 235` retraction pointed to.** `latest_generation_predicate` (`store/skill_store.py`) is the ONE rule every reader now applies (store methods AND the three raw-SQL API readers in `api/routes/models.py`/`api/routes/stations.py`, which previously used no filter at all or a THIRD, independent `freshness == "current"` rule) — unifying three incompatible "what is current" definitions the plan's audit found. `generation_id` is deliberately NOT a foreign key (score/diagram rows are written WHILE their generation is still being computed; the ledger row is inserted LAST, by `PgSkillStore.publish_generation`, only after every expected write already succeeded — this IS the completeness gate, since publish is a single, atomic `INSERT`). `mark_stale` is now documented as production-unsafe (D2c: a real `UPDATE`, `sapphire_worker` holds `INSERT` only, no grant added) and `publish_generation` with `score_count=diagram_count=0` is its INSERT-only replacement. `compute_skills_task`/`compute_skills_flow`'s `hindcast_run_id`, and the new `compute_combined_skills_task`/`compute_combined_skills_flow`'s `hindcast_run_ids: dict[ModelId, UUID]`, are now REQUIRED (T3) — omitting either is a `TypeError`, not a silent 1970–2100 unscoped fetch. Station onboarding's own `fetch_hindcasts` call (`services/onboarding.py::_compute_skill`) was ALSO fixed opportunistically (plan-scoped as non-blocking, but contained enough to do here): `onboard_model` (`services/model_onboarding.py`) now mints ONE `hindcast_run_id` shared between its Step 5 (`_run_hindcast`) and Step 6 (`_compute_skill`) calls, so the skill leg scopes its own fetch instead of reading this station/model's entire training-period history. See `docs/decisions/plan-235-skill-generations.md` for the full design record, and `docs/plans/228-hindcast-and-skill-on-wrong-data.md` D3 (now unblocked — mark via `publish_generation`, then recompute, as one operation).
