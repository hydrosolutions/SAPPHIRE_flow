...
- **Host startup (Mac mini)**: `scripts/launchd/*.plist` → `start-sapphire.sh` (the sole `docker compose … up -d` per reboot), `install-launchd.sh`, `bootstrap-mac-mini.sh`; the watchdog surface = `src/sapphire_flow/ops/watchdog.py` + `scripts/launchd/watchdog.sh` + operator-created host secrets `secrets/slack_webhook_url` + `secrets/deadman_url` (Plan 158 D3) + a manually-installed `newsyslog` log-rotation conf. The `cicd.md` systemd unit is an **illustration, not shipped**. **Plan 158 D2/D2b (T2): `install-launchd.sh` now has a per-label launchd *domain*** — `ch.hydrosolutions.sapphire-watchdog` is a `daemon` (`/Library/LaunchDaemons`, system-wide, requires root, installed via `--label`); the stack-starter and prune jobs stay `agent` (`gui/$(id -u)`) until their own host cutover (D5/T5). The installer refuses to escalate privileges for a daemon label itself — it skips (agent-only sweep) or hard-fails (`--label` explicitly given) and always prints the `sudo …` re-run. A root run with NO `--label` (a full sweep) is rejected outright — under `sudo`, `$HOME` resolves to root's home, mismatching the `gui/<SUDO_UID>` domain the agent plists would be bootstrapped into. Before a system-domain bootstrap it also boots out any stale `gui/<uid>` registration of the same label and **aborts (never bootstraps `system/…`) if that bootout fails or a post-bootout `launchctl print` still finds it** — the one-domain invariant is verified, not just attempted. `--dry-run` previews any label's target domain without touching `plutil`/`launchctl` (not a full transcript of every real-run action). **Plan 158 T1c F1: a successful daemon install also removes the leftover agent-domain plist FILE** from `AGENTS_DIR` (not merely the current registration) — launchd re-scans that directory at every login, so a leftover file would otherwise silently reload as a duplicate LaunchAgent racing the daemon on the same state file; a FAILED daemon bootstrap rolls back to the saved legacy plist so a half-migration never leaves the host with no watchdog running. **F2: an explicit `--label` for an *agent*-domain job is also refused under `sudo`** (not just a full sweep) for the identical `$HOME`-resolves-to-root's-home reason. `bootstrap-mac-mini.sh --uninstall` mirrors the domain-aware verification: it checks actual `launchctl print` registration (not plist-file presence) in both domains, refuses (nonzero exit) to remove a `system/`-domain registration without root, and exits nonzero — never claims "uninstall complete" — if anything is still registered afterwards. **Plan 158 T1c F4: uninstall now boots out the stack-starter + prune jobs FIRST, then stops and POSITIVELY VERIFIES the Docker stack (`docker compose ps -q`, never reading a failed check as "no containers"), and boots out the watchdog LAST**; Docker Desktop is per-user, so the compose calls route through the configured service account (`SAPPHIRE_SERVICE_ACCOUNT`, default `sapphire`) via `sudo -u`, and a mismatched `SUDO_USER` under `sudo` is refused outright rather than silently operating in the wrong session (which would see zero containers and falsely "succeed"). **Plan 158 D8/T4: every launchd job that shells out to `docker` resolves its binary + `DOCKER_HOST` from one sourced snippet, `scripts/launchd/docker-endpoint.sh`** (`SAPPHIRE_DOCKER_BIN`/`SAPPHIRE_DOCKER_HOST`, defaulting to the Docker-Desktop values proven in production; `DOCKER_CMD` — the pre-existing test seam — still wins over both) — was three independently hardcoded `/usr/local/bin/docker` / `unix:///var/run/docker.sock` values, two of which would not resolve to a Homebrew-installed Colima CLI (Plan 159).
