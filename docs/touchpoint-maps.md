...
- **A refused forecast must not be alerted on.** `store_forecast`'s return value is discarded by `flows/run_forecast_cycle.py` and Phase C alerts on the in-memory ensemble, so the flow withholds a refused `(station, model, parameter)` from alert selection (`_drop_refused_ensembles`). A new store-refusal path must do the same or the cycle acts on content the store would not keep.
