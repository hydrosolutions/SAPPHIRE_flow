...
- **`eval_period_end` is now the per-run identity in both natural keys (third-review BLOCKER fix, migration 0052 amended in place, no new migration).** `computation_version` is a static algorithm/schema marker (bumped only when the scoring CODE changes) — it does NOT change between two legitimately different computations of the same stratum (e.g. today's and next week's `compute_combined_skills_task` run, both scoring `POOLED_MODEL_ID`/`model_artifact_id=NULL`), so without a per-run identity the later recomputation's `INSERT ... ON CONFLICT DO NOTHING` silently discarded the corrected score forever. `eval_period_end` (an existing, always-populated column) is fixed for every row one `compute_skill_for_station`/`compute_combined_skill` call produces and strictly later for a later recomputation — a retry of the same run still collides (idempotent), a later recomputation is a NEW row. **The migration's earlier destructive `DELETE`-based dedup (blocker found on the third review) is GONE** — deleting rows to make room for a tightened unique index violates `docs/standards/cicd.md`'s additive-only migration rule; adding `eval_period_end` to the key means no legitimate historical row collides in the first place, and a genuine duplicate now fails the `CREATE UNIQUE INDEX` loudly instead of being silently discarded. `PgSkillStore.fetch_latest_scores`/`fetch_latest_diagrams` resolve "latest" PER STRATUM via a window function (`MAX(eval_period_end) OVER (PARTITION BY <every natural-key column except eval_period_end>)`) — a global max would make a cohort with a smaller eval window (e.g. an hourly cadence next to a daily one) vanish from "latest" results entirely just because another stratum's window is larger; `tests/fakes/fake_stores.py::FakeSkillStore` mirrors the same per-stratum logic. Proved by `tests/integration/store/test_skill_store.py::test_recomputation_with_changed_score_supersedes_without_losing_history`, proven RED against the pre-fix key (the corrected score was silently dropped, only 1 of 2 rows survived).
