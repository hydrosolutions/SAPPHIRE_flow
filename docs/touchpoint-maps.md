...
- **Skill scoring compares a forecast step-mean against an observation resampled to the SAME step (Plan 228 P2 fix)** — `compute_skill_for_station` (`services/skill/service.py`) infers the forecast's own `time_step` from the hindcast data itself (`_infer_forecast_time_step`: the lead-to-lead gap inside one hindcast's ensemble, falling back to the issue-to-issue gap for a horizon-1 model) and resamples observations to it (`_resample_observations_to_forecast_step`, reusing `resample_to_time_step` + `aggregation_method_for` — the SAME aggregation the model trains against, e.g. MEAN for discharge) before the `_build_strata` join. Before the fix, `_build_strata` looked a forecast's `valid_time` up in a dict keyed by RAW observation timestamps — comparing a daily-mean forecast against whichever single instantaneous reading happened to land exactly on `valid_time` (measured live: median 6.4%, p95 48.6%, max 78.8% error, present in every skill score regardless of forecast quality). The resample is anchored to a `valid_time` drawn from the hindcasts themselves (review fixer round) — without it, buckets fall on UTC-midnight boundaries and a non-midnight `valid_time` never exact-matches any bucket key, so `_build_strata` would silently find NO observation for any forecast at all, not merely a wrong one. When `time_step` cannot be inferred (a single hindcast with a single lead step), `compute_skill_for_station` now returns no scores (`compute_skill_for_station.time_step_unknown_no_scores`) instead of falling back to the raw instantaneous join — that fallback was the exact P2 defect reintroduced silently, with only a log line to mark it; no score is preferable to a wrong one.
