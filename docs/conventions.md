...
| `UnsupportedModelRequirementError` | ForecastInterface `InputRequirement` declares non-empty `future_known` in more than one `time_step` branch (Plan 156, raised at adapter construction) — SAP3 domain types are single-resolution; OR a requirement with more than one `time_step` branch at all reaches an actual predict/train call (Plan 156 follow-up, raised at delivery instead — SAP3 can only deliver one branch's dynamic inputs per call, so the accepted one-future-forced-plus-past-only shape still fails loudly rather than silently omitting the past-only branch) | `discover_models()` skips the entry point and logs, but keeps discovering the rest; every other `ConfigurationError` still darkens the whole registry (see `services/model_registry.py::discover_models`). The delivery-time raise propagates as a normal `SapphireError` through predict/train (Plan 130 carve-out: the training call catches broad `Exception`) |
...
> **Plan 130 carve-out — the training call is a broad `except Exception`, not `SapphireError`-only**: `train_models_flow`'s `_train_model_task` call (T.3) and `onboard_model_flow`'s `_train_onboarding_model_task` call (M.3) each wrap ONLY the training call in `try: ... except Exception as exc:`, recording the unit as failed (`TrainingResult.error` / `OnboardingOutcome.FAILED_TRAINING`) and continuing with the remaining units — deliberately broader than the `SapphireError`-only rule above. Rationale: a model's `train()` can raise a bare `TypeError`/`ValueError` for an anticipated data condition (e.g. `nwp_regression`'s missing-future-value tail gap, or its bare `ValueError` for insufficient aligned samples) that is not (and need not be) wrapped in a `SapphireError` subclass — and one unit's training bug must never abort an entire multi-station/multi-model run. The guard is scoped to the training call ONLY: a SHA-256 artifact-integrity mismatch discovered just after (a system-level corruption signal, not a per-unit data condition) still propagates and aborts the run, as do exceptions from any other phase (hindcast, skill compute, assignment) — those keep the original `SapphireError`-only / unexpected-propagates convention.
...
| `parameters.aggregation_method` / `AggregationMethod` | `sum`, `mean` — the DB `CHECK` constraint (`db/metadata.py`) and this table's rows are unchanged. **`AggregationMethod` itself also has `max` (Plan 228 review fixer round)**, added for `ModelDataRequirements.declared_aggregations` — a model's per-variable FI-declared aggregation, kept in-memory only (never written to this column) and fed straight to `resample_to_time_step` (`services/training_data.py::resolved_aggregation_methods`). Do not persist `AggregationMethod.MAX` into `parameters.aggregation_method` without a migration widening the `CHECK` constraint first. | — | v0+v1 |
