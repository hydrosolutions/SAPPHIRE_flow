...
| 3 | **Flow 6 → 7 → 8** — Train → hindcast → skill | Auto-promote (no approval gate). Full skill metric suite (CRPS, CRPSss, BSS, POD/FAR/CSI, peak timing, NSE, KGE, PBIAS, MAE, diagrams). **Every `skill_scores`/`hindcast_forecasts` row predating 2026-09-01 is invalid** — computed before the Plan 228 fix to the hindcast lookback resampling (P1) and the skill/observation join (P2); see `docs/decisions/plan-228-hindcast-skill-resampling.md`. |
