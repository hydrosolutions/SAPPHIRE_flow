...
- v0 = Swiss public data, up to ~170 stations (LINDAS-available BAFU gauges), single VM, 1-2 users. Architecture supports scaling to ~1000 stations across deployments. **Resolved (Plan 175 D5 -> Plan 186, `docs/decisions/bafu-lindas-rate-limit.md`):** the operational ingest's per-station LINDAS polling did NOT scale to that ceiling — LINDAS's measured burst limit is 3 requests, so one-request-per-station-per-poll was structurally impossible past a handful of stations. Plan 186 replaced it with one whole-graph SPARQL query per ingest run (the same approach the quarantined Plan 136 archive collector already used, sharing only the query text/a non-raising key helper — see D2), filtered locally to onboarded stations. Ingest cost is now flat in station count.
...
| 3 | **Flow 6 → 7 → 8** — Train → hindcast → skill | Auto-promote (no approval gate). Full skill metric suite (CRPS, CRPSss, BSS, POD/FAR/CSI, peak timing, NSE, KGE, PBIAS, MAE, diagrams). **Every `skill_scores` row at `computation_version < 2` is invalid** (P2, the observation join, ran unresampled for every model regardless of parameter) — this is a version cutoff, not the 2026-09-01 plan date: the in-flight mac-mini test run kept producing `computation_version = 1` scores past that date while the fix sat unmerged. **Only the `hindcast_forecasts` rows produced by `linear_regression_daily` and `nwp_regression`** predate the P1 lookback-resampling fix and are invalid on that count (a weather-only or no-`past_targets` model, e.g. `nwp_rainfall_runoff`/`climatology_fallback`, was never affected by P1; `persistence_fallback` has a smaller, related error — see the plan). `evaluate_skill_gate` only ever considers the highest `computation_version` among CURRENT rows, so a stale v1 score can no longer decide model promotion even before D3's mark-and-recompute runs. See `docs/decisions/plan-228-hindcast-skill-resampling.md`. |
| 3 | **Flow 13** — Model onboarding | Implemented as `onboard_model_flow` in code. Register + validate + smoke test + train + hindcast + skill gate + auto-promote. Sample model (LinearRegressionDaily). No approval gate (auto-promote). Reuses services from Flows 6/7/8 directly (does NOT call `train_models` flow — composes the underlying service layer to interpose the skill gate). **External-artifact import (Plan 157 T3)** is a SEPARATE, standalone `import-model-artifact` deployment (`flows/import_model_artifact.py`) for an artifact SAP3 did not train — it never enters Flow 13's train/hindcast/skill-gate path and records external provenance (`model_artifact_provenance`) instead. |
...
| Flow 4 — Pipeline monitoring | v0c or v1 | **→ DECISION (plan 013, amended by Plan 100)**: Manual supervision suffices at Swiss v0 scale (~170 stations). Health endpoint (`/api/v1/health`) does lightweight live checks (DB ping, Prefect heartbeat) independently — does not require Flow 4. Plan 100 adds a minimal `/api/v1/health/detail` read endpoint and dashboard page for any `pipeline_health` records written by forecast resilience checks before full Flow 4 exists. **Plan 116** adds one such resilience check ahead of full Flow 4: `PipelineCheckType.FORECAST_FRESHNESS`, a v0 CYCLE-LEVEL contract — a forecast cycle that persists zero forecasts (in `forecast_values`) emits a CRITICAL record (`detail={"forecasts_stored": int}`, `subject="forecast_cycle"`), OK otherwise. **Plan 223** adds an optional `detail.reason` — present only on an NWP-fetch-abort CRITICAL record, a short sanitised code (never a raw exception) naming the cause, e.g. `"nwp_file_count_exceeded: 501 > 500"` — rendered by the watchdog's Slack alert when present. A fatal mid-cycle group `store_forecast` failure (Plan 116 fixer round) also forces CRITICAL even when `forecasts_stored > 0` — some forecasts stored before the cycle crashed, but the cycle did not complete normally, so it must not read as healthy — immediately before the exception re-propagates out of the flow. Backfill/replay runs (explicit `cycle_time`) emit none. This answers only "did the cycle produce ANYTHING" — the per-(station, model, parameter) coverage ledger `docs/architecture-context.md`'s `forecast_freshness` detail schema still names as the v1-vision target shape is EXPLICITLY DEFERRED (a coverage ledger of that shape drew blockers across two independent reviews on the Plan 158 branch; a separate, later decision), not built by Plan 116. |
...
**Plan 098 addition (v0b)**: v0 also runs a second `ingest` pool served by a dedicated `prefect-worker-ingest` container, purely to isolate the `*/5` observation ingest from the shared `default` pool. This is an obs-feed-isolation measure, not a change to the general-worker model. **Plan 176 addition**: `collect-bafu-observations` also routes to `ingest` — insurance against a cadence regression rather than a repair, since T0 measured pickup latency at ≤3 s even on `default` (see `docs/plans/archive/176-lindas-archive-completeness.md` § Execution isolation). Every other flow (including `ingest-weather-history`) stays on `default`. The three-pool ops/training/hindcast topology remains a v1 concern.
...
### A11a. Future-snow (JSNOW) forecast forcing channel (Nepal v1, Recap Gateway)

**Full design**: a station's future-snow forcing (`swe`, `snow_depth`, `snowmelt`) is fetched, stored, and broadcast alongside the IFS ensemble exactly like precipitation/temperature — deterministic (`member_id=None`) rows broadcast across every real IFS member present in the batch.

**v0/Swiss**: not applicable — MeteoSwiss/ICON-CH2-EPS carries no snow-forecast channel; `SnowForecastSource` is a capability Protocol only the Recap Gateway adapter satisfies, so this is inert on Swiss deployments.

**Nepal v1 (Plan 145)**: `RecapGatewayForecastAdapter.fetch_snow_forecast` is wired into the Flow-1 Phase-A task (`_fetch_nwp_task`), capability-gated via `isinstance(adapter, SnowForecastSource)` — never a concrete-class import, so MeteoSwiss/replay/test doubles are unaffected. Station-level scoping is pre-computed by the flow (`_compute_required_snow`) from the ACTIVE station assignments' resolved models' own `future_dynamic_features`, intersected with `RECAP_VARIABLES`' `SNOW_CANONICAL_PARAMETERS` — a station with no snow-requiring active assignment triggers zero `snow.forecast` calls. The snow fetch rides the SAME resolved IFS cycle (`WeatherForecastResult.cycle_time` from the just-completed IFS fetch), never a second `resolve_latest_cycle` probe. A `(hru, canonical variable)` gap (Gateway `source_data_missing`/`subscription_not_found`) is contained inside `fetch_snow_forecast` — rows for other variables in the same HRU are preserved, and the gap is folded into the typed `SnowForecastFetchResult.unavailable` map, which surfaces as the `snow_unavailable` cycle outcome (DEGRADED health, never a cycle-wide `nwp_unavailable` abort). Post-fetch, `_reconcile_snow_coverage` (review fold-in) additionally compares what was REQUIRED against what actually arrived per bound station — catching a resolver-skipped required station (the resolver only fails loud when ALL stations are unmappable) and a successful-but-zero-row/partial response (no exception, empty `unavailable`) that neither of the above signals can see on its own; either gap also sets `snow_unavailable=True`. The `assemble_station_operational_inputs` `return None` trap on an empty future-NWP read is relaxed to log-and-continue with an empty `future_dynamic` frame, so the existing per-model `assess_future_coverage` gate — not this assembly guard — is what suppresses a snow-fed (or IFS-fed) model while a non-NWP fallback still forecasts. Snow units are SET as of Plan 219 (`hs` m->cm, `rof`/`swe` mm identity), replacing `convert=None`. **Units CONFIRMED by the snow modeller's own specification (2026-09-07):** every NetCDF variable carries CF metadata — `swe: mm`, `hs: m`, `rof: mm` — plus `cell_methods` (`time: point` for the SWE/depth STATES, `time: sum` for runoff, i.e. `rof` is the hourly increment and a daily total is the sum of 24). Our converters (`hs` m→cm ×100, `rof`/`swe` identity) and our aggregation (`swe`/`snow_depth` MEAN, `snowmelt` SUM) both match that spec. Time axes are UTC throughout. **Residual risk, narrowed:** the source files carry the units, but whether the Gateway's parquet extraction preserves that attribute end-to-end is not something we can see from our side — the modeller flagged the same. Our factors assume the Gateway serves the source units unchanged. A later factor change would still NOT correct rows already stored — the store ignores repeats and `value` is not part of the natural key. The antecedent/PAST snow channel (reanalysis-fed) is a separate, later plan (146).

### A11b. Antecedent (past) snow reanalysis channel (Nepal v1, Recap Gateway)

**Full design**: a station's antecedent/lookback snow forcing (`swe`, `snow_depth`, `snowmelt`) is fetched from the recap-Gateway JSNOW reanalysis endpoint by a **dedicated ingest flow/deployment** (`ingest-recap-reanalysis`, Plan 146 D1/D2), persisted to `HistoricalForcingStore` under the `ForcingSource.RECAP_SNOW_REANALYSIS` provenance tag (Plan 146 D3), and read-routed into `past_dynamic` for training/hindcast/live inference via the hybrid reanalysis chain (Plan 146 D4).

**v0/Swiss**: not applicable — the deployment registers the flow unconditionally, but on a Swiss deployment (no recap-reanalysis-bound stations, no `[adapters.recap_gateway]` section) it resolves zero in-scope stations every run and returns a benign no-op before ever touching Recap config — see docs/standards/orchestration.md § Rolling-window ingest flows.

**Nepal v1 (Plan 146)**: `RecapGatewayReanalysisAdapter.fetch_snow_reanalysis` (a NEW non-Protocol method, mirroring `fetch_snow_forecast`'s reasoning on the reanalysis side) fetches the FULL snow-variable ceiling (`swe`/`snow_depth`/`snowmelt`) for every in-scope recap-reanalysis HRU, unconditionally — the ingest flow is **model-agnostic**, with per-HRU *subscription* discovered at RUNTIME via `RecapSnowUnavailableError.code` (`subscription_not_found` = permanent/structural, excluded from WARNING; `source_data_missing` = transient, contributes to WARNING). No `ModelStore`/`StationGroupStore` injection, no requirement resolution. The ingest flow runs on a **fixed rolling window** (`window_days` default 21, no persisted watermark) with health measured by EFFECT via `HistoricalForcingStore.fetch_covered_days` per `(station_id, parameter)` — finer than the sibling's `fetch_latest_valid_time` because all three snow variables share ONE `source` literal. Read-side: `swe`/`snow_depth`/`snowmelt` are added as single-source chains (`RECAP_SNOW_REANALYSIS`) to the hybrid reader's `_PRIORITY_CHAINS` **and** `DEFAULT_PARAMETERS` — the latter is required, not optional, since every real construction-time caller (training/hindcast/live + six more) relies on the default scope. Snow units are SET as of Plan 219 (shared with A11a — the SAME converters, so this reanalysis path converts too) — 146 verifies the read-path PLUMBING with an injected test model; it does **not** itself onboard a real snow-fed FI model (canonical-schema expansion + unit resolution + onboarding feature-availability is an explicit Plan 139 dependency). Backfill for a newly-bound station is the SAME flow run with a wide `window_days` + an explicit `station_ids` subset (mechanism shipped by 146; depth-sufficiency/before-operational enforcement is a Plan 139/onboarding-flow follow-on).

### A12. ML lookback forcing: MeteoSwiss self-derived, basin-averaged gridded data (CAMELS-CH retired as the live source)
...
**v0a/early v0b (plan 021)**: Used basin-averaged forcing extracted from gridded data — CAMELS-CH for training, ICON-CH2-EPS (via GridExtractor) for operational inference. **→ DECISION (Plan 115b4 §5B/§5D/§5E, supersedes Plan 021's CAMELS-CH-for-training decision)**: past-dynamic (training/hindcast/live) forcing now resolves through the `hybrid` reader's per-parameter priority chain over **self-derived MeteoSwiss products on SAP3's own basin polygons** — `RhiresD → RprelimD` (precipitation), `TabsD` (temperature), `TminD`/`TmaxD` (temperature min/max), `SrelD` (relative sunshine duration) — with **no CAMELS-CH tier**. `DeploymentConfig.reanalysis_source` defaults to `"hybrid"`. CAMELS-CH is now a **validation reference + audit trail** (Plan 115b3) — its `historical_forcing` rows are retained and readable via a direct source-keyed fetch, and no live forecast/training/hindcast path reads it (the hybrid chain never wires a camels-ch tier). The `camels-ch` weather-source *binding* itself is retired (row deleted, migration `0033`) by Plan 115b5, as a SEPARATE, LATER release gated on Release A's staging deploy-gate — not in the same deploy as this flip. CAMELS-CH remains the v0 **runoff/discharge**, **static-attribute**, and **basin-polygon** source — only the *weather* (forcing) role moved. The forcing source is injected via adapter dependency, not hardcoded — `prepare_model_inputs()` and training data assembly accept a forcing source parameter (see §I2). **v1**: Switch to ERA5-Land via `WeatherReanalysisSource` Protocol for Nepal (Plan 183 §I2 has the first concrete implementation, `Era5LandReanalysisAdapter` — reading `sloth-dynamic` directly for v0 Swiss validation, not yet the Data-Gateway-mediated production Nepal path).
...
**Tier 2 — Synthetic realistic (integration/e2e)**: 7 BAFU reference stations, 2 years hourly discharge + water_level (5 stations), deterministic generator (`tests/fixtures/reference/generate_fixtures.py`, seeded RNG). Realistic seasonal/diurnal patterns with injected QC anomalies (spikes, frozen sensor, range violations). ~1.7 MB in `tests/fixtures/reference/bafu_observations.parquet`. **Deviation from original spec**: replaces "Real CAMELS-CH" and "recorded ICON-CH2-EPS" — CAMELS-CH provides only daily data (pipeline needs hourly), BAFU LINDAS serves real-time only (no historical archive). NWP fixtures deferred to v0b alongside gridded pipeline. When operational deployment accumulates 6+ months of real observations, re-record with actual data (see `docs/plans/058-bafu-lindas-archive-collection.md` for the collection + promotion plan). **Plan 176 (2026-08-18)**: the quarantined BAFU LINDAS observation archive collector (`flows/collect_bafu_observations.py`, Plan 136) now captures the network's real **10-minute publish grid** (not hourly — Plan 176 D1/D2 fixed the collector's cadence and dedup key after both were found to be measuring/matching the wrong interval), gzipped. This is a **forward-only** archive — LINDAS serves no history, so sub-daily real observations exist starting from this plan's deployment date, not retroactively; relevant to v0b sub-daily model R&D once enough real history accumulates. Still quarantined (keyed on `(gauge_code, lindas_kind)`, no `station_id`), still not a source for the Tier-2 synthetic fixture above.
...
**Rule**: Training data gathering (Flow 6 step T.2) and forecast input preparation (Flow 1 step 1.7) must accept a forcing source dependency (adapter), not directly query a specific data source. The `WeatherReanalysisSource` Protocol exists but is not wired into v0's training/inference pipeline — the injection point must still be present.

**Plan 183 (2026-08-18)**: `WeatherReanalysisSource` now has a concrete implementation — `Era5LandReanalysisAdapter` (`adapters/era5_land_reanalysis.py`) reads ERA5-Land basin-averaged forcing directly from `s3://sloth-dynamic/v1/era5/`, the SAME store aquacast's training lineage (`aquaire`) reads. This exists to **validate** v0 Swiss models against the training-lineage forcing (Plan 183 T3, Caravan climate-index parity) — it is selectable via `DeploymentConfig.reanalysis_source = "era5_land"` (`select_reanalysis_source`, Plan 183 T4) but v0 (Swiss, MeteoSwiss `hybrid` default) has no reason to select it. It is **distinct** from the Data-Gateway-mediated ERA5-Land path this section and `architecture-context.md` describe for production Nepal ingest (CDS API via SAPPHIRE Data Gateway) — that remains a separate, later integration; Plan 183 does not implement it.
