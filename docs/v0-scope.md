...
**Plan 098 addition (v0b)**: v0 also runs a second `ingest` pool served by a dedicated `prefect-worker-ingest` container, purely to isolate the `*/5` observation ingest from the shared `default` pool. This is an obs-feed-isolation measure, not a change to the general-worker model. **Plan 176 addition**: `collect-bafu-observations` also routes to `ingest` — insurance against a cadence regression rather than a repair, since T0 measured pickup latency at ≤3 s even on `default` (see `docs/plans/176-lindas-archive-completeness.md` § Execution isolation). Every other flow (including `ingest-weather-history`) stays on `default`. The three-pool ops/training/hindcast topology remains a v1 concern.
