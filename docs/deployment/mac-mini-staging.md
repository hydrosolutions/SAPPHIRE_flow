...
| `could not read the 'services' block out of 'launchctl print …'` | the sweep could not extract a COMPLETE top-level `services` table — **UNKNOWN**, not clean. Two causes: the dump had no such table (most likely `launchctl print`'s undocumented format changed), or the table opened and never closed (a TRUNCATED dump — the rows past the cut are invisible, so a partial read must not pass as clean) | run `launchctl print gui/$(id -u)` by hand. If it shows a whole `services` table, the earlier dump was truncated — just re-run `--uninstall`. If there is no `services` table at all, the format changed; file it |
